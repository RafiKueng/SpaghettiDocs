...
General user interface actions, like showing dialog screens or tooltips, are handled by \lmt{ui.html}.
Standard low level events like \E{onClick} on buttons trigger according high level events.
The module \lmt{ui.html} makes use of the features of jQuery\footnote{and it's plugin jQuery UI}, in order to show and to style the elements in the HTML DOM that represent the dialog boxes, buttons and so on.
