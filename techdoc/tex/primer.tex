...
It offers many new features, including the Canvas element (discussed in \secref{canvas}) element and the integration of \T{SVG} (\secref{svg}).
...
SVG graphics can be rendered to a canvas using the external JS library \T{canvg.js}\footnote{Developped by Gabe Lerner with contributions to the CSS engine by Rafael Küng.}.
